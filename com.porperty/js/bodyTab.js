...
layui.define(["element", "jquery"], function (exports) {
    ...
    Tab.prototype.render = function () {
        var url = this.tabConfig.url, isLocal = this.tabConfig.isLocal;
        $.ajax({
            url: url, islocal: isLocal, success: function (data) {
                //显示左侧菜单
                if ($(".navBar").html() == '') {
                    var _this = this;
    ...
    Tab.prototype.tabAdd = function (_this) {
        ...
        if (_this.attr("target") == "_blank") {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                if (_url == "javascript:_norole()") {
                    layer.msg("暂无此选项查看权限！");
                    return;
                }

                _url += _url.indexOf("?") > 0 ? "&" : "?";
                _url += "&runder_key=" + (+new Date());

                element.tabAdd(tabFilter, {
                    title: title,
                    content: "<iframe src='" + _url + "' data-id='" + tabIdIndex + "'></frame>",
                    id: new Date().getTime()
                })
